...
class AnimalNER:
    ...
    def predict(self, text):
        ...
        Properly handles subword tokens (BPE tokenization used by RoBERTa).
        """
        # Split into words first - same as in training pipeline
        words = text.split()
        ...
        for word_id in sorted(word_labels.keys()):
            ...
            if label == 'B-ANIMAL':
                ...
                if current_entity_words:
                    entities.append(
                        self._create_entity(words, current_entity_words, word_offsets)
                    )
                # Start new entity
                current_entity_words = [word_id]

            elif label == 'I-ANIMAL':
                # Continue current entity (only if we have B-ANIMAL)
                if current_entity_words:
    ...
    def _create_entity(self, words, word_ids, word_offsets):
        """
        Create entity dict from words.
        word_ids: list of word IDs that belong to the entity
        word_offsets: dict {word_id: {'start': ..., 'end': ...}}
        """
        # Reconstruct entity text directly from word list
        entity_words = [words[wid] for wid in word_ids]
        ...
        with open(args.input_file, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f, 1):
                ...
                if line:
                    ...
                    print(f"\n{i}. Text: {line}")
                    if entities:
                        print(f"Animals: {[e['entity'] for e in entities]}")
                    else:
                        print("No animals found")
    

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Animal NER Inference')
