        ...
        if entity_tokens:
            for j, token_idx in enumerate(entity_tokens):
                if j == 0:  # First token gets B- tag
                    ner_tags[token_idx] = 'B-ANIMAL'
                else:       # Subsequent tokens get I- tags
                    ner_tags[token_idx] = 'I-ANIMAL'
...
class AnimalNERDataset(torch.utils.data.Dataset):
    """Custom dataset for Animal NER task"""
    def __init__(self, data, tokenizer, max_len=128):
        self.data = data
        self.tokenizer = tokenizer
        self.max_len = max_len
        self.label2id = {'O': 0, 'B-ANIMAL': 1, 'I-ANIMAL': 2}
        self.id2label = {0: 'O', 1: 'B-ANIMAL', 2: 'I-ANIMAL'}
    ...
    def __getitem__(self, idx):
        ...
        # add_prefix_space=True is required for RoBERTa tokenizer
...
def evaluate(model, dataloader, device, id2label):
    ...
    animal_true_tokens = sum(1 for seq in true_labels for tag in seq if tag != 'O')
    animal_pred_tokens = sum(1 for seq in predictions for tag in seq if tag != 'O')
    sequences_with_animals = sum(1 for seq in true_labels if any(tag != 'O' for tag in seq))
    
    print(f"\nDETAILED STATISTICS")
    print(f"Total sequences: {len(true_labels)}")
    print(f"Sequences with animals: {sequences_with_animals}")
    print(f"Real animal tokens: {animal_true_tokens}")
    print(f"Predicted animal tokens: {animal_pred_tokens}")
    ...
    for i, (true, pred) in enumerate(zip(true_labels, predictions)):
        if any(tag != 'O' for tag in true):
            print(f"Example {animal_examples + 1}:")
            print(f"  True: {true}")
            print(f"  Pred: {pred}")
            animal_examples += 1
            if animal_examples >= 3:
                break
...
def main(args):
    ...
    # add_prefix_space=True is required for correct RoBERTa tokenization
    ...
    train_dataset = AnimalNERDataset(train_data, tokenizer, max_len=args.max_len)
    val_dataset = AnimalNERDataset(val_data, tokenizer, max_len=args.max_len)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train Animal NER model')
    parser.add_argument('--dataset_path', type=str, required=True, help='Path to animal dataset CSV')
    parser.add_argument('--output_dir', type=str, default='./checkpoints/ner_model', help='Output directory for model')
